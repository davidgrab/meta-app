...
combined_forest_plot <- function(results, options) {
  ...
  data <- data.frame(
    ...
    yi = c(
      safe_extract(results$random, "TE"),
      safe_extract(results$fixed, "TE"),
      safe_extract(results$bivariate, "yi")
    ),
    ci.lb = c(
      safe_extract(results$random, "lower"),
    ...
    ci.ub = c(
      safe_extract(results$random, "upper"),
...
method_comparison_plot <- function(random_result, fixed_result, bivariate_result) {
  ...
  p <- ggplot(data, aes(x = Effect, y = Method)) +
    ...
    theme_minimal() +
    labs(
      title = paste("Comparison of", effect_measure, "Estimates"),
        ...
        metaregression_results <- tryCatch(
          {
            ...
            list(
              model = metareg_model, plot = bubble_plot, moderator_type = moderator_type, moderator_name = moderator_var,
...
try(source("R/replicability.R"), silent = TRUE)
...
6. **Replicability Analysis** - Evaluates the likelihood of effects replicating in new studies (Jaljuli et al., 2021).
...
- **Replicability**: High r-values (>=2) suggest findings are robust and likely to replicate.
...
```{r rep-summary, echo=FALSE, results="asis"}
safe_run({
  if (!is.null(params$replicability_result) && !is.null(params$consistency_status)) {
    r_val <- round(params$replicability_result$r.value, 4)
    status <- params$consistency_status

    cat("**R-Value (r(2)):** ", r_val, "\n\n")
    cat("**Consistency Status:** ", status$label, "\n\n")
    cat("**Direction Bounds:** u_R =", status$u_max_R, " / u_L =", status$u_max_L, "\n\n")

    cat("The r-value estimates the number of studies expected to replicate. Values >= 2 provide strong evidence of replicability.")
  } else {
    cat("Replicability analysis data not available.")
  }
})
```

## Replicability Forest Plot

```{r rep-forest, fig.width=12, fig.height=8}
safe_run({
  if (!is.null(params$replicability_result)) {
    # Ensure grid graphics (used by metarep) are captured
    render_replicability_forest(params$replicability_result)
  }
})
```
...
shapiro_test_random <- function(random_results) {
  ...
  blup_test <- if (length(valid_blups) >= 3 && length(valid_blups) <= 5000) {
    ...
    list(
      statistic = NA, p.value = NA, method = "Shapiro-Wilk normality test",
    ...
    biv_minus_i <- tryCatch(
      {
        if (input$data_type == "smd") {
          ...
          metabiv(
            studlab = data_minus_i$study, sm = "SMD",
            y = data_minus_i$smd, sigma2 = var_minus_i, verbose = FALSE
          )
        } else {
          metabiv(
            event.e = data_minus_i$ie, n.e = data_minus_i$it,
...
print_normality_summary <- function(diagnostics, model_type) {
  ...
  if (model_type == "fixed") {
    ...
    if (!is.na(diagnostics$shapiro_test$p.value)) {
      cat(
        "  W =", round(diagnostics$shapiro_test$statistic, 4),
    ...
    if (!is.na(diagnostics$shapiro_tests$blup_test$p.value)) {
      cat(
        "  W =", round(diagnostics$shapiro_tests$blup_test$statistic, 4),
    ...
    if (!is.na(diagnostics$shapiro_tests$deleted_residuals_test$p.value)) {
      cat(
        "  W =", round(diagnostics$shapiro_tests$deleted_residuals_test$statistic, 4),
...
calculate_bivariate_deleted_residuals <- function(bivariate_results, data, input) {
  ...
  for (i in 1:k) {
    ...
    if (input$data_type == "smd") {
      ...
      res_loo <- metabiv(
        event.e = data_loo$ie, n.e = data_loo$it,
...
calculate_threshold_probabilities <- function(bivariate_result, custom_thresholds = NULL) {
  ...
  if (sm == "SMD") {
    ...
    threshold_labels <- c(
      "–0.10 (potential harm)", "0.00 (any benefit)",
    ...
    threshold_labels <- c(
      "0.90 (potential benefit)", "1.00 (no effect)",
...
calculate_threshold_probabilities_from_cdf <- function(CDF.ci.obj, custom_thresholds = NULL, sm = NULL,
  ...
  safe_approx <- function(x, y, xout) {
    ...
    if (sum(uniq) < 2) {
      return(rep(mean(y_ord), length(xout)))
    }
