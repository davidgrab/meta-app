...
source("R/replicability.R") # Replicability analysis functions

# server.R
server <- function(input, output, session) {
  # Enable thematic for auto-theming plots based on bslib theme
  ...
  observe({
    print("Hiding tabs")
    lapply(c("Overall Results", "Random Effects Analysis", "Fixed Effects Analysis", "JCR Method", "Replicability Analysis", "Meta-Regression"), function(tab) {
  ...
  output$enhancedBaujatPlot <- renderPlotly({
    ...
    p <- plot_ly(
      ...
      layout(
        title = paste("Baujat Plot (", effect_measure_label(), ")"),
  ...
  calculate_influence_bivariate <- function(biv_res, full_data, input) {
    ...
    influence <- lapply(1:length(studylab), function(i) {
      ...
      res_i <- if (input$data_type == "smd") {
        ...
        metabiv(
          event.e = loo_data$ie, n.e = loo_data$it,
          event.c = loo_data$pe, n.c = loo_data$pt,
          studlab = loo_data$study, sm = input$effect_measure, verbose = FALSE
        ) # No logs
      }
  ...
  output$bivariateGRADESummary <- renderPrint({
    ...
    if (bivariate_result()$tau > 0.5) {
      grade_score <- grade_score - 2
  ...
  metaregression_results <- eventReactive(input$run_metaregression, {
    ...
    if (input$moderator_type == "categorical" || is.character(moderator_var) || is.factor(moderator_var)) {
      ...
      rma_method <- if (input$use_random_effects) {
        switch(input$het_estimator,
          "DL" = "DL",
          "PM" = "PM",
          "REML" = "REML",
          "ML" = "ML",
  ...
  output$metaregressionInterpretation <- renderPrint({
    ...
    if (moderator_significant) {
      ...
      if (results$moderator_type == "continuous") {
        ...
        cat(
          "Interpretation: For every 1-unit increase in ", results$moderator_name,
          ", the effect size changes by ", round(slope, 4), " units.\n\n"
        )

        if (slope > 0) {
          cat(
            "Direction: Positive association - higher values of ", results$moderator_name,
            " are associated with larger effect sizes.\n"
          )
        } else {
          cat(
            "Direction: Negative association - higher values of ", results$moderator_name,
  ...
  output$bivariateSubgroupForestPlot <- renderPlot(
    {
      ...
      plots <- lapply(names(results), function(nm) {
        grid::grid.grabExpr({
          tryCatch(
            {
              forest.metabiv(results[[nm]],
                title = paste("Subgroup:", nm),
                xlab = paste("Effect Size (", effect_measure_label(), ")")
              )
            },
            error = function(e) {
              plot(1, type = "n", main = paste("Plot unavailable (", nm, ")"))
              text(1, 1, e$message)
            }
          )
        })
  ...
  output$downloadReportFile <- downloadHandler(
    ...
    content = function(file) {
      withProgress(message = "Generating report...", value = 0, {
        ...
        # Safely retrieve replicability results if they exist
        rep_result <- tryCatch(rvalueResult(), error = function(e) NULL)
        cons_status <- tryCatch(consistencyStatus(), error = function(e) NULL)
        ...
        output_file <- render_report(
          ...
          replicability_result = rep_result,
  ...
  # Compute r-value when button is clicked (minimal replicability u=2 for headline)
  rvalueResult <- eventReactive(input$run_rvalue, {
    print("Run R-value button clicked")
    req(combinedResults()$random)
    print("Combined results available")
    tryCatch(
      {
        print(paste("Computing r-value with u=2, t=", input$rvalue_t, "common=", input$rvalue_common_effect))
        res <- compute_rvalue(
          combinedResults()$random,
          u = 2,
          t = input$rvalue_t,
          common_effect = input$rvalue_common_effect
        )
        if (is.null(res)) print("compute_rvalue returned NULL") else print("compute_rvalue success")
        res
      },
      error = function(e) {
        print(paste("Error in rvalueResult:", e$message))
        showNotification(paste("Error computing r-value:", e$message), type = "error")
        NULL
      }
    )
  })

  # Helper to get consistency status
  consistencyStatus <- reactive({
    req(rvalueResult())
    print("Calling see consistencyStatus")
    get_consistency_status(rvalueResult())
  })

  # Generate Replicability Statement
  output$replicabilityStatement <- renderText({
    req(rvalueResult())
    k <- nrow(data())
    get_replicability_statement(rvalueResult(), k)
  })

  # Generate Analysis Settings Metadata
  output$analysisSettings <- renderUI({
    req(combinedResults()$random, input$rvalue_t)
    k <- nrow(data())
    alpha <- input$rvalue_t
    ci_level <- if (!is.null(input$confidence_level)) input$confidence_level else 95
    mode <- if (input$rvalue_common_effect) "Common-effect (Pearson)" else "Assumption-free"

    HTML(sprintf(
      "<strong>k = %d studies</strong>, <strong>α = %s</strong>, <strong>CI level = %s%%</strong>, <strong>Mode: %s</strong>",
      k, alpha, ci_level, mode
    ))
  })

  # 1) Summary Card Outputs
  output$val_r2 <- renderText({
    req(rvalueResult())
    round(rvalueResult()$r.value, 4)
  })

  output$val_direction <- renderText({
    req(consistencyStatus())
    s <- consistencyStatus()
    # If one direction dominates, show it. Otherwise "Mixed".
    if (s$u_max_R > s$u_max_L) {
      "Increased"
    } else if (s$u_max_L > s$u_max_R) {
      "Decreased"
    } else {
      "Mixed / None"
    }
  ...
  output$val_bounds <- renderText({
    req(consistencyStatus())
    s <- consistencyStatus()
    sprintf("u_R: %d / u_L: %d", s$u_max_R, s$u_max_L)
  })

  output$card_consistency <- renderUI({
    req(consistencyStatus())
    stat <- consistencyStatus()

    icon_name <- switch(stat$color,
      "success" = "check-circle-fill",
      "warning" = "exclamation-triangle-fill",
      "secondary" = "question-circle-fill",
      "info-circle-fill"
    )
  ...
  # 2) r(u) Curve Plot
  output$ruCurvePlot <- renderPlot({
    req(combinedResults()$random)
    req(input$run_rvalue)
    ...
    # Compute for u = 2..n
    diag_data <- get_ru_diagnostics(combinedResults()$random, n_studies)
    plot_ru_curve(diag_data)
  })

  # 3) Diagnostic Table
  output$ruTable <- renderDT({
    req(combinedResults()$random)
    req(input$run_rvalue)
    ...
    # Identify u* (largest u with p <= 0.05)
    u_star <- max(diag_data$u[diag_data$r_u <= 0.05], na.rm = TRUE)
    if (!is.finite(u_star)) u_star <- -1

    # Format p-values nicely (< 0.0001)
    diag_data$r_u_fmt <- format.pval(diag_data$r_u, eps = 1e-4, digits = 4)

    table_len <- min(n_studies, 15)

    datatable(diag_data[, c("u", "r_u_fmt")],
      colnames = c("u", "r(u) p-value"),
      options = list(
        pageLength = table_len,
        dom = "t",
        scrollY = "400px",
        paging = FALSE
      ),
      ...
      formatStyle(
        "u",
        target = "row",
        backgroundColor = styleEqual(u_star, "#d1f2eb"), # Highlight the row for u*
        fontWeight = styleEqual(u_star, "bold")
      )
  })

  # 4) Forest Plot
  output$rvalueForestPlot <- renderPlot(
    {
      req(input$run_rvalue)
      rval <- rvalueResult()
      render_replicability_forest(rval)
    },
    height = function() {
      req(data())
      450 + nrow(data()) * 20
  ...
  # Info modal for r-value
  observeEvent(input$rvalue_info, {
    showModal(modalDialog(
      title = "About R-Value Analysis",
      HTML(paste0(
        "<p>The <strong>r-value</strong> is a replicability statistic that estimates the minimum ",
        "number of studies whose findings are expected to replicate in an independent sample.</p>",
        "<p><strong>How it works:</strong></p>",
        "<ul>",
        "<li>Computes p-values for each study's effect estimate</li>",
        "<li>Uses the selected threshold (u) to identify 'significant' studies</li>",
        "<li>Applies the r-value algorithm from the metarep package</li>",
        "</ul>",
        "<p><strong>Interpretation:</strong></p>",
        "<ul>",
        "<li>r ≥ 2: Strong evidence that at least 2 studies have replicable effects</li>",
        "<li>r ≥ 1: Evidence that at least 1 study has a replicable effect</li>",
        "<li>r < 1: Insufficient evidence for replicable effects</li>",
        "</ul>",
        "<p><strong>Reference:</strong> Jaljuli et al. - 'Quantifying replicability and consistency in systematic reviews'</p>"
      )),
      easyClose = TRUE,
      footer = modalButton("Close")
    ))
  ...
  # Info modal for consistency analysis
  observeEvent(input$consistency_info, {
    showModal(modalDialog(
      title = "About Consistency Analysis",
      HTML(paste0(
        "<p><strong>Consistency analysis</strong> evaluates how well studies agree with each other.</p>",
        "<p><strong>Metrics:</strong></p>",
        "<ul>",
        "<li><strong>Direction Consistency:</strong> Proportion of studies showing effects in the majority direction</li>",
        "<li><strong>Significance Consistency:</strong> Proportion of studies with statistically significant results (p < 0.05)</li>",
        "<li><strong>Pooled Agreement:</strong> Proportion of studies with effects in the same direction as the pooled estimate</li>",
        "</ul>",
        "<p><strong>Interpretation:</strong></p>",
        "<ul>",
        "<li>≥80%: High consistency</li>",
        "<li>50-80%: Moderate consistency</li>",
        "<li><50%: Low consistency (mixed findings)</li>",
        "</ul>"
      )),
      easyClose = TRUE,
      footer = modalButton("Close")
