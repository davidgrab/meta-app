...
## Joint Confidence Region (JCR) Method

The Joint Confidence Region (JCR) method is an advanced meta-analysis approach that is implemented in this application based on the methodology described in Saad et al. (2019) and the supplementary material provided. This method assumes that the observed effect sizes (`y_k` for study `k`) arise from a normal distribution with a study-specific mean (`θ_k`) and within-study variance (`σ_k^2`). The study-specific means `θ_k` are themselves assumed to be a random sample from a normal distribution with a common mean `μ` (the overall effect size) and between-study variance `τ^2`.
...
The parameters `μ` and `τ` are estimated jointly using Maximum Likelihood Estimation (MLE). This joint estimation approach is what distinguishes the JCR method from traditional random-effects meta-analysis, which typically estimates τ first and then μ conditionally.

### Effect Measure Handling

The JCR method is adapted for different effect measures:
...
*   **JCR Forest Plot**: Similar to the other forest plots, but the overall summary diamond is based on the JCR method's joint MLE estimates for `μ` and its confidence interval.
*   **Confidence Region Plot**: This is the signature plot of the JCR approach. It shows the joint confidence region for the two main parameters of the model: the overall effect size `μ` and the between-study standard deviation `τ`. The plot is generated by calculating the deviance for a grid of `μ` and `τ` values and then finding the contour lines corresponding to specific p-values (e.g., p=0.05 for a 95% confidence region). The Maximum Likelihood Estimates (MLE) for `μ` and `τ` are marked on the plot. This joint estimation and visualization is what makes the JCR method distinct from traditional approaches.
...
*   **Q-Q Plot (Standardized Residuals)**: This plot assesses the normality assumption for the random effects using the jointly estimated parameters. It plots the quantiles of the standardized residuals against the quantiles of a standard normal distribution. The standardized residual for study `k` is calculated as:
    \[
    r_k = \frac{y_k - \hat{\mu}_{MLE}}{\sqrt{\sigma_k^2 + \hat{\tau}_{MLE}^2}}
    \]
    where both μ̂ and τ̂ are estimated jointly through MLE.
*   **Q-Q Plot (Raw Residuals)**: This plot shows the raw residuals (`y_k - \hat{\mu}_{MLE}`) against the theoretical quantiles. The size of the points is proportional to the study size, giving more visual weight to larger studies.

### Publication Bias

*   **Adapted Funnel Plot**: This is a standard funnel plot of the effect sizes (`y_k`) against their standard errors (`σ_k`), but using the more precise estimates from the JCR method's joint MLE approach.

### Sensitivity Analysis

*   **Confidence Region Shift Plot**: This is a novel plot for sensitivity analysis in the JCR method. It shows how the 95% joint confidence region for (`μ`, `τ`) shifts when each study is removed one at a time. This helps to visually identify influential studies that cause a large shift in the confidence region.
*   **Enhanced Baujat Plot**: This plot is adapted for the JCR method to identify influential studies. It plots a measure of a study's contribution to heterogeneity against its influence on the pooled estimate, using the joint MLE estimates.

This completes the theoretical overview of the methods implemented in the app. The use of established libraries for standard methods and a custom implementation for the advanced Joint Confidence Region (JCR) method provides a robust and comprehensive tool for meta-analysis. 