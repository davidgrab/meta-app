...
run_comparison <- function(dataset_name, file_path, mle_start_values) {
  ...
  benchmark_mle <- comp.tau.mu.log.RR.MLE(data_tbl, mle_start_values)
...
run_or_comparison <- function(dataset_name, file_path) {
  ...
  # 2. Benchmark using meta::metabin with sm="OR"
  cat("--- Benchmark Values from meta::metabin (sm='OR') ---\n")
  metabin_result <- metabin(
    event.e = data_tbl$event.e,
    n.e = data_tbl$n.e,
    event.c = data_tbl$event.c,
    n.c = data_tbl$n.c,
    sm = "OR",
    method = "MH" # Using Mantel-Haenszel as a standard
  )
  # metabin stores results in TE (log OR) and tau2
  cat("Estimated mu (log OR):", metabin_result$TE.random, "\n")
  cat("Estimated tau:", sqrt(metabin_result$tau2), "\n\n")
